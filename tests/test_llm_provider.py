...
class LLMProviderTestCase(unittest.TestCase):
    ...
    def test_stage1_prompt_requires_memory_update_extraction_status(self) -> None:
        ...
        self.assertIn("memory_update_extraction.reason is always required", prompt)
        self.assertIn("memory_update_extraction.reason must be a non-empty string", prompt)
        self.assertIn("Never return an empty string for memory_update_extraction.reason", prompt)
        self.assertIn("reason must briefly say what was extracted", prompt)
        self.assertIn("reason must briefly say why no memory-relevant update was extracted", prompt)
