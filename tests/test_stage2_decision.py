...
class Stage2DecisionTestCase(unittest.TestCase):
    ...
    def test_valid_final_answer(self) -> None:
        decision = Stage2Decision(final_answer="Provider returned a final answer.", memory_update_extraction=self._fail_extraction())
    ...
    def test_final_answer_empty_fails(self) -> None:
        with self.assertRaises(ValidationError):
            Stage2Decision(final_answer="   ", memory_update_extraction=self._fail_extraction())
    ...
    def test_extra_summary_field_fails(self) -> None:
        with self.assertRaises(ValidationError):
            Stage2Decision(final_answer="Done.", memory_update_extraction=self._fail_extraction(), summary="not allowed")

    def test_schema_version_exists(self) -> None:
        decision = Stage2Decision(final_answer="Done.", memory_update_extraction=self._fail_extraction())
    ...
    def test_empty_memory_candidates_with_ok_memory_update_extraction_fails(self) -> None:
        with self.assertRaises(ValidationError):
            Stage2Decision(final_answer="Done.", memory_update_extraction=self._ok_extraction())
